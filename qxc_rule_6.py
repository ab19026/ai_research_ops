...
def process(count, maxConsecutiveCount, seq, nearGapStat, gapStat, prob_cand_arr, pred, avg_window, data_len, good_count, range_stat, name, label, label_dim):
    ...
    if count['big'] >= 7 * 1.5:
        if len(big_tmp) == gapStat[0.02]:
            if big_gap_tmp[0] < 0.0241 and big_gap_tmp[1] > 0.0261:
                arr.add(big_tmp[0])
            ...
            if big_gap_tmp[0] < 0.022:
                arr.add(big_tmp[0])
            if big_gap_tmp[0] < 0.0235 and big_gap_tmp[1] > 0.026:
                arr.add(big_tmp[0])
        ...
        if midJudge(big_tmp, gapStat, 0.03, True):
            ...
            for i in range(pos -2, -1, -1):
                if big_gap_tmp[pos - 1] - big_gap_tmp[i] < 0.0001:
                    arr.add(big_tmp[i])
            pos2 = gapStat[0.03] - 1
            arr.add(big_tmp[pos2])
            for i in range(pos2 - 1, -1 ,-1):
                if big_gap_tmp[pos2] - big_gap_tmp[i] < 0.00001:
                    arr.add(big_tmp[i])
            if len(big_tmp) - gapStat[0.03] <= 2:
            ...
            if big_gap_tmp[0] < 0.0255 and big_gap_tmp[0] > 0.0249 and big_gap_tmp[1] > 0.027:
                arr.add(big_tmp[0])
        ...
        if midJudge(big_tmp, gapStat, 0.02, True):
            ...
            for i in range(pos - 2, -1, -1):
                if big_gap_tmp[pos - 1] - big_gap_tmp[i] < 0.0001:
                    arr.add(big_tmp[i])
            if big_gap_tmp[pos - 1] > 0.0185 and pos - 2 > -1:
                arr.add(big_tmp[pos - 2])
        ...
        if midJudge(big_tmp, gapStat, 0.06, False):
            ...
            for i in range(pos + 1, len(big_tmp)):
                if big_gap_tmp[i] - big_gap_tmp[pos] < 0.0001:
                    arr.add(big_tmp[i])
            if big_gap_tmp[pos - 1] > 0.0575:
                if pos - 2 > -1:
                    arr.add(big_tmp[pos - 2])
                for i in range(pos - 3, -1, -1):
                    if big_gap_tmp[pos - 2] - big_gap_tmp[i] < 0.0001:
                        arr.add(big_tmp[i])
        if midJudge(big_tmp, gapStat, 0.07, True):
            if gapStat[0.08] <= 2:
                findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=False)
        ...
        if midJudge(big_tmp, gapStat, 0.07, False):
            ...
            if big_gap_tmp[pos] > 0.069:
                ...
                for i in range(pos - 1, -1, -1):
                    if big_gap_tmp[pos] - big_gap_tmp[i] < 0.0001:
                        arr.add(big_tmp[i])
            if big_gap_tmp[pos + 1] < 0.072 and pos + 3 < len(big_tmp) and big_gap_tmp[pos + 3] > 0.0788:
                arr.add(big_tmp[pos + 2])
        ...
        if midJudge(big_tmp, gapStat, 0.05, False):
            ...
            gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos, left=False)
            if big_gap_tmp[pos] < 0.051 and pos + 1 < len(big_tmp):
                arr.add(big_tmp[pos + 1])
            else:
                arr.add(big_tmp[pos])
            if big_gap_tmp[pos - 1] < 0.0492:
                arr.add(big_tmp[pos - 1])
            else:
                arr.add(big_tmp[pos - 1])
                arr.add(big_tmp[pos - 2])
        ...
        if len(small_near_tmp) >= len(big_near_tmp):
            if len(small_near_tmp) == 2:
                ...
                if nearGapStat[-0.01] == 0:
                    if small_near_gap_tmp[1] > -0.0012:
                        arr.add(small_near_tmp[0])
                    if small_near_gap_tmp[-1] > -0.005 and small_near_gap_tmp[0] < -0.005 or small_near_gap_tmp[0] == small_near_gap_tmp[1]:
                        arr.add(small_near_tmp[0])
                        arr.add(small_near_tmp[1])
            if nearGapStat[-0.01] == 0:
                ...
                if len(small_near_gap_tmp) > 0 and small_near_gap_tmp[0] < -0.009:
                    arr.add(small_near_tmp[0])
                if noConsecutiveGap(small_near_gap_tmp):
                    if len(small_near_tmp) > 1 and small_near_gap_tmp[0] < -0.007 and small_near_gap_tmp[1] > -0.005:
                        arr.add(small_near_tmp[0])
        ...
        if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) - len(small_near_tmp) >= -1:
            ...
            if len(big_near_tmp) == 2:
                ...
                if nearGapStat[0.01] == 1:
                    arr.add(big_near_tmp[0])
                if nearGapStat[0.01] == 0:
                    if big_near_gap_tmp[1] == big_near_gap_tmp[0]:
                        arr.add(big_near_tmp[0])
                        arr.add(big_near_tmp[1])
                    else:
                        arr.add(big_near_tmp[0])
                    if big_near_gap_tmp[1] > 0.008 and big_near_gap_tmp[0] < 0.001:
                        arr.add(big_near_tmp[1])
            if nearGapStat[0.01] == 0:
                if len(big_near_gap_tmp) > 1 and big_near_gap_tmp[-1] > 0.0087:
                    arr.add(big_near_tmp[-1])
            ...
            if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                pos = len(big_near_gap_tmp) - nearGapStat[0.01]
        ...
        print('大的全都要')
    #参差不齐接近
    elif count['near'] >= int(5 * 1.5) and maxConsecutiveCount['near'] < 4 * 1.5:
        if len(seq['small']) + len(seq['smallNear']) >= 7 * 1.5:
            ...
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) - len(big_near_tmp) >= -1:
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    if small_near_gap_tmp[-1] > -0.0015:
            ...
            if len(big_near_tmp) == 2:
                ...
                if big_near_gap_tmp[0] < 0.012:
                    if big_near_gap_tmp[0] == big_near_gap_tmp[1]:
                        arr.add(big_near_tmp[0])
                        arr.add(big_near_tmp[1])
            print('参差不齐接近时小的偏多' + extraSmall(seq, 1.5))
        elif len(seq['big']) + len(seq['bigNear']) >= 7 * 1.5:
            if len(big_tmp) >= len(small_tmp) and len(big_tmp) >= len(small_near_tmp):
                if len(big_tmp) == gapStat[0.02]:
                    if midJudge(big_tmp, gapStat, 0.04, False):
                        ...
                        if big_gap_tmp[-1] < 0.05 and big_gap_tmp[-1] > 0.0485:
                            arr.add(big_tmp[-1])
                if midJudge(big_tmp, gapStat, 0.04, False):
                ...
                if midJudge(big_tmp, gapStat, 0.02, True):
                    ...
                    gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos + 1, left=False)
                    gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos, left=True)
                    findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=False, consecutive_at_left=True, choose_single=True)
                if midJudge(big_tmp, gapStat, 0.03, True):
                    pos = len(big_tmp) - gapStat[0.03] - 1
                    arr.add(big_tmp[pos])
                    for i in range(1, len(big_tmp)):
                        if i < len(big_tmp) and i + 2 < len(big_tmp) and big_gap_tmp[i] > 0.03 and big_gap_tmp[i] < 0.033 and big_gap_tmp[i+2] > 0.037 and big_gap_tmp[i+2] < 0.04:
                            arr.add(big_tmp[i+1])
                            break
                    arr.add(big_tmp[pos + 1])
                ...
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    ...
                    for i in range(pos + 1, len(big_near_tmp)):
                        if big_near_gap_tmp[i] - big_near_gap_tmp[pos] < 0.0001:
                            arr.add(big_near_tmp[i])
                    if nearGapStat[0.01] < 3:
                        findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=True, choose_single=False)
            ...
            if len(small_near_tmp) - len(big_near_tmp) >= -1:
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, True):
                    if small_near_gap_tmp[1] > -0.012 and small_near_gap_tmp[0] < -0.013:
                        arr.add(small_near_tmp[0])
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    pos = nearGapStat[-0.01] - 1
                    arr.add(small_near_tmp[pos])
            ...
            if len(small_tmp) == 2:
                ...
                if gapStat[-0.04] == 1:
                    arr.add(small_tmp[1])
            if len(small_near_tmp) > 1 and small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                for v in small_near_tmp:
                    arr.add(v)
            if len(small_tmp) == 1:
                if gapStat[-0.03] == 0:
                    arr.add(small_tmp[0])
            print('参差不齐接近时大的偏多' + extraBig(seq, 1.5))
            ...
            if len(small_tmp) >= len(big_tmp) and len(small_tmp) - len(big_near_tmp) >= -1:
                ...
                if len(small_tmp) == gapStat[-0.03]:
                    if small_gap_tmp[-1] > -0.0308:
                        arr.add(small_tmp[-1])
                if midJudge(small_tmp, gapStat, -0.04, False):
                    pos = gapStat[-0.04] - 1
                    gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=True)
                if midJudge(small_tmp, gapStat, -0.05, False):
                    if small_gap_tmp[1] > -0.052:
                        arr.add(small_tmp[0])
                if midJudge(small_tmp, gapStat, -0.04, False):
                    pos = gapStat[-0.04]
                    arr.add(small_tmp[pos])
                ...
                if midJudge(small_tmp, gapStat, -0.03, False):
                    ...
                    if gapStat[-0.04] == 0:
                        pos = gapStat[-0.03] - 1
                        if small_gap_tmp[pos] > -0.032 and pos - 1 > -1:
                            arr.add(small_tmp[pos - 1])
                        else:
                            arr.add(small_tmp[pos + 1])
                    else:
                ...
                if midJudge(small_tmp, gapStat, -0.02, True):
                    ...
                    for i in range(pos + 2, len(small_tmp)):
                        if small_gap_tmp[i] - small_gap_tmp[pos + 1] < 0.00001:
                            arr.add(small_tmp[i])    
            elif len(small_tmp) >= len(big_tmp):
                if len(small_tmp) == 2:
                    if gapStat[-0.04] == 1:
                        arr.add(small_tmp[1])
            if len(big_near_tmp) - len(small_near_tmp) >= -2 and len(big_near_tmp) - len(small_tmp) >= -2:
                if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                    pos = len(big_near_tmp) - nearGapStat[0.01]
                    gapAndConsecutive(arr, big_near_tmp, big_near_gap_tmp, pos, left=False)
                ...
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    ...
                    if noConsecutiveGap(big_near_gap_tmp):
                        if big_near_gap_tmp[0] < 0.005 and big_near_gap_tmp[1] > 0.003:
                            arr.add(big_near_tmp[0])
                if nearGapStat[0.01] == 0:
                    ...
                    if big_near_gap_tmp[0] < 0.001:
                        arr.add(big_near_tmp[0])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=False, choose_single=True)
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=True)
            elif len(big_near_tmp) >= 3:
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    pos = len(big_near_tmp) - nearGapStat[0.01]
                    arr.add(big_near_tmp[pos])
                    for i in range(pos + 1, len(big_near_tmp)):
                ...
                if nearGapStat[0.01] <= 1:
                    arr.add(big_near_tmp[1])
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) - len(big_near_tmp) >= -2:
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=True, choose_single=False)
                    ...
                    if small_near_gap_tmp[pos - 1] > -0.011 and pos - 2 > -1:
            ...
            print('参差不齐接近时小的比大的多一些' + extraSmall(seq, 1.5))
        elif len(seq['small']) + len(seq['smallNear']) < len(seq['big']) + len(seq['bigNear']):
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) - len(small_near_tmp) >= -2:
                if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                    pos = len(big_near_tmp) - nearGapStat[0.01]
                    gapAndConsecutive(arr, big_near_tmp, big_near_gap_tmp, pos, left=False)
                ...
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    if big_near_gap_tmp[0] <= 0.0051 and big_near_gap_tmp[1] > 0.006:
                        arr.add(big_near_tmp[0])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=True, choose_single=False)
                    ...
                    for i in range(pos - 2, -1, -1):
                        if big_near_gap_tmp[pos - 1] - big_near_gap_tmp[i] < 0.0001:
                            arr.add(big_near_tmp[i])
                    if noConsecutiveGap(big_near_gap_tmp):
                        if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[1] >= 0.0049:
                            arr.add(big_near_tmp[0])
                if nearGapStat[0.01] == 0:
                    if big_near_gap_tmp[-1] > 0.0092:
                        arr.add(big_near_tmp[-2])
                    if big_near_gap_tmp[0] < 0.001 and big_near_gap_tmp[1] > 0.001:
                        arr.add(big_near_tmp[1])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=True)
                ...
                if nearGapStat[0.01] == len(big_near_tmp):
                    ...
                    if big_near_gap_tmp[0] == big_near_gap_tmp[1]:
                        ...
                        for i in range(1, len(big_near_tmp)):
                            if big_near_gap_tmp[i] - big_near_gap_tmp[0] < 0.00001:
                                arr.add(big_near_tmp[i])
            ...
            if len(big_tmp) - len(small_tmp) >= -1 and len(big_tmp) - len(small_near_tmp) >= -1:
                ...
                if midJudge(big_tmp, gapStat, 0.03, False):
                    ...
                    arr.add(big_tmp[pos])
                if midJudge(big_tmp, gapStat, 0.04, False):
                    if gapStat[0.05] == 0 and big_gap_tmp[-1] > 0.0481:
                        arr.add(big_tmp[-1])
                    pos = len(big_tmp) - gapStat[0.04] - 1
                    arr.add(big_tmp[pos])
                    if pos + 1 < len(big_tmp):
                        ...
                        for i in range(pos + 2, len(big_tmp)):
                            if big_gap_tmp[i] - big_gap_tmp[pos + 1] < 0.0001:
                                arr.add(big_tmp[i])
                if len(big_tmp) == gapStat[0.02]:
                    if big_gap_tmp[0] < 0.0235:
                        arr.add(big_tmp[0])
            elif len(big_tmp) - len(small_tmp) >= -2 and len(big_tmp) >= len(small_near_tmp):
                if midJudge(big_tmp, gapStat, 0.02, False):
                    pos = len(big_tmp) - gapStat[0.02]
                    arr.add(big_tmp[pos])
            if len(small_tmp) == 1:
            ...
            if len(small_tmp) == 2:
                ...
                if gapStat[-0.02] == 2:
                    if gapStat[-0.03] <= 1:
                        arr.add(small_tmp[1])
            if len(small_near_tmp) - len(big_tmp) >= -2 and len(small_near_tmp) - len(big_near_tmp) >= -2:
                if small_near_gap_tmp[-1] > -0.0004:
                    arr.add(small_near_tmp[-1])
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    if small_near_gap_tmp[0] < -0.014 and small_near_gap_tmp[1] > -0.012:
                        arr.add(small_near_tmp[0])
                    if small_near_gap_tmp[-1] > -0.005 and small_near_gap_tmp[-2] < -0.0051:
                        arr.add(small_near_tmp[-2])
                    elif small_near_gap_tmp[-1] > -0.004 and small_near_gap_tmp[-2] > -0.0051:
                        arr.add(small_near_tmp[-1])
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=True, choose_single=True)
                    pos = nearGapStat[-0.01] - 1
                    arr.add(small_near_tmp[pos])
                ...
                if nearGapStat[-0.01] == 0:
                    ...
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
            elif len(small_near_tmp) - len(big_near_tmp) >= -2:
                if nearGapStat[-0.01] == 0:
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=False)
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
            ...
            if len(small_tmp) - len(big_tmp) >= -2 and len(small_tmp) - len(big_near_tmp) >= -2:
                ...
                if midJudge(small_tmp, gapStat, -0.03, True):
                    ...
                    if small_gap_tmp[pos] > -0.0305:
                        ...
                        arr.add(small_tmp[pos])
            elif len(small_tmp) - len(big_tmp) >= -1:
                if midJudge(small_tmp, gapStat, -0.02, True):
                    pos = gapStat[-0.02] - 1
                    arr.add(small_tmp[pos])
                if midJudge(small_tmp, gapStat, -0.04, True):
            ...
            if len(small_tmp) == 1:
                if gapStat[-0.04] == 0:
                    arr.add(small_tmp[0])
            print('参差不齐接近时大的比小的多一些' + extraBig(seq, 1.5))
        else:
            print('参差不齐接近时大小一样')
    #连续接近
    elif count['near'] >= int(5 * 1.5) and maxConsecutiveCount['near'] >= 4 * 1.5:
        if len(seq['small']) + len(seq['smallNear']) >= 7 * 1.5:
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) >= len(big_near_tmp):
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    pos = nearGapStat[-0.01]
                    if small_near_gap_tmp[pos] < -0.009:
            ...
            if len(small_tmp) >= len(big_tmp) and len(small_tmp) >= len(big_near_tmp):
                ...
                if midJudge(small_tmp, gapStat, -0.05, True):
                    ...
                    for i in range(pos - 1, -1, -1):
                        if small_gap_tmp[pos] - small_gap_tmp[i] < 0.00001:
                            arr.add(small_tmp[i])
            print('连续接近时小的偏多' + extraSmall(seq, 1.5))
        elif len(seq['big']) + len(seq['bigNear']) >= 7 * 1.5:
            if len(small_near_tmp) > len(big_tmp):
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    pos = nearGapStat[-0.01] - 1
                    gapAndConsecutive(arr, small_near_tmp, small_gap_tmp, pos, left=True)
            print('连续接近时大的偏多' + extraBig(seq, 1.5))
        elif len(seq['small']) + len(seq['smallNear']) > len(seq['big']) + len(seq['bigNear']):
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) - len(big_near_tmp) >= -2:
                if nearGapStat[-0.01] == 0:
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=True, choose_single=True)
            ...
            if len(small_tmp) >= len(big_tmp) and len(small_tmp) >= len(big_near_tmp):
                ...
                if midJudge(small_tmp, gapStat, -0.05, True):
                    pos = gapStat[-0.05]
                    if small_gap_tmp[pos] < -0.048:
                        if pos + 1 < len(small_tmp):
                            arr.add(small_tmp[pos + 1])
            print('连续接近时小的比大的多一些' + extraSmall(seq, 1.5))
        elif len(seq['small']) + len(seq['smallNear']) < len(seq['big']) + len(seq['bigNear']):
            ...
            print('连续接近时大的比小的多一些' + extraBig(seq, 1.5))
        else:
            print('连续接近时大小一样')
    elif count['small'] >= int(5 * 1.5) and maxConsecutiveCount['small'] < 4 * 1.5:
        if midJudge(small_tmp, gapStat, -0.07, False):
            pos = gapStat[-0.07] - 1
            gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=True)
        if midJudge(small_tmp, gapStat, -0.06, False):
            if small_gap_tmp[0] < -0.068:
                arr.add(small_tmp[0])
            if small_gap_tmp[1] > -0.062:
                arr.add(small_tmp[0])
            pos = gapStat[-0.06]
            arr.add(small_tmp[pos])
            arr.add(small_tmp[pos - 1])
        ...
        if midJudge(small_tmp, gapStat, -0.04, True):
            ...
            arr.add(small_tmp[pos])
            if gapStat[-0.05] == 0:
                if small_gap_tmp[0] == small_gap_tmp[1] and small_gap_tmp[1] == small_gap_tmp[2]:
                    if small_gap_tmp[0] < -0.049:
                        arr.add(small_tmp[0])
                        for i in range(1, len(small_tmp)):
                            if small_gap_tmp[i] - small_gap_tmp[0] < 0.00001:
                                arr.add(small_tmp[i])
            arr.add(small_tmp[pos - 1])
            for i in range(pos - 2, -1, -1):
                if small_gap_tmp[pos - 1] - small_gap_tmp[i] < 0.0001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[pos - 1] > -0.0415:
                arr.add(small_tmp[pos - 2])
        if midJudge(small_tmp, gapStat, -0.03, False):
            pos = gapStat[-0.03]
            arr.add(small_tmp[pos])
            ...
            for i in range(pos - 2, -1, -1):
                if small_gap_tmp[pos - 1] - small_gap_tmp[i] < 0.00001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[pos - 1] > -0.0318 and pos - 2 > -1:
                arr.add(small_tmp[pos - 2])
        if midJudge(small_tmp, gapStat, -0.03, True):
        ...
        if midJudge(small_tmp, gapStat, -0.02, True):
            ...
            if small_gap_tmp[pos] < -0.018:
                if pos + 1 < len(small_tmp):
                    arr.add(small_tmp[pos + 1])
            if small_gap_tmp[pos - 1] > -0.0201:
                arr.add(small_tmp[pos - 2])
            if small_gap_tmp[pos - 1] > -0.021 and small_gap_tmp[pos - 3] < -0.029:
                arr.add(small_tmp[pos - 2])
        ...
        if len(big_near_tmp) >= len(small_near_tmp) - 1:
            if big_near_gap_tmp[0] == big_near_gap_tmp[-1]:
                for v in big_near_tmp:
                    arr.add(v)
            if nearGapStat[0.01] == 0:
                if big_near_gap_tmp[0] < 0.00098:
                    arr.add(big_near_tmp[0])
                if noConsecutiveGap(big_near_gap_tmp):
                    if big_near_gap_tmp[0] < 0.0015 and len(big_near_tmp) > 1:
                        arr.add(big_near_tmp[1])
                else:
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=True)
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=False)
        ...
        if len(small_near_tmp) - len(big_tmp) >= -1 and len(small_near_tmp) - len(big_near_tmp) >= -1:
            if len(small_near_tmp) == 2:
                if small_near_gap_tmp[0] < -0.0075 and small_near_gap_tmp[1] > -0.0056:
                    arr.add(small_near_tmp[1])
                if nearGapStat[-0.01] <= 1:
                    arr.add(small_near_tmp[0])
            ...
            if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                ...
                findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=True, choose_single=True)
                findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=True, consecutive_at_left=False, choose_single=False)
                if small_near_gap_tmp[-1] > -0.003 and small_near_gap_tmp[-2] < -0.004:
                    arr.add(small_near_tmp[-2])
                elif small_near_gap_tmp[-1] > -0.003 and small_near_gap_tmp[-2] > -0.004:
                    arr.add(small_near_tmp[-1])
            if midJudge(small_near_tmp, nearGapStat, -0.01, True):
                pos = nearGapStat[-0.01] - 1
                arr.add(small_near_tmp[pos])
            ...
            if nearGapStat[-0.01] == len(small_near_tmp):
                if small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                    for v in small_near_tmp:
                        arr.add(v)
            if nearGapStat[-0.01] == 0:
                if len(small_near_tmp) > 2 and small_near_gap_tmp[0] == small_near_gap_tmp[1] and small_near_gap_tmp[1] == small_near_gap_tmp[2]:
                    arr.add(small_near_tmp[0])
                    arr.add(small_near_tmp[1])
                    arr.add(small_near_tmp[2]) 
                if noConsecutiveGap(small_near_gap_tmp):
                    if small_near_gap_tmp[0] < -0.009:
                        arr.add(small_near_tmp[1])
                    if small_near_gap_tmp[-1] > -0.0018:
                        arr.add(small_near_tmp[-2])
            ...
            if len(small_near_tmp) == 2:
                if small_near_gap_tmp[0] < -0.0075:
                    arr.add(small_near_tmp[1])
            if small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                for v in small_near_tmp:
                    arr.add(v)
        ...
        if len(big_tmp) == 3:
            ...
            if midJudge(big_tmp, gapStat, 0.03, False):
                pos = len(big_tmp) - gapStat[0.03] - 1
                arr.add(big_tmp[pos])
        if len(big_tmp) - len(small_near_tmp) >= -1:
            if midJudge(big_tmp, gapStat, 0.02, False):
                pos = len(big_tmp) - gapStat[0.02]
                gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos, left=False)
            if midJudge(big_tmp, gapStat, 0.03, False):
                pos = len(big_tmp) - gapStat[0.03]
                arr.add(big_tmp[pos])
                if big_gap_tmp[-1] < 0.04 and big_gap_tmp[-1] > 0.035:
                    arr.add(big_tmp[-1])
            if len(big_tmp) == 1:
                if gapStat[0.02] == 0:
                    arr.add(big_tmp[0])
        print('参差不齐小' + restExcSmall(seq, 1.5))
    elif count['big'] >= int(5 * 1.5) and count['near'] >= 2 * 1.5:
        if len(big_tmp) >= len(small_tmp) and len(big_tmp) >= len(small_near_tmp):
            if midJudge(big_tmp, gapStat, 0.05, False):
                pos = len(big_tmp) - gapStat[0.05] - 1
                if big_gap_tmp[pos] > 0.0466:
                    arr.add(big_tmp[pos - 1])
                arr.add(big_tmp[pos])
                for i in range(pos - 1, -1, -1):
                    if big_gap_tmp[pos] - big_gap_tmp[i] < 0.0001:
            ...
            if midJudge(big_tmp, gapStat, 0.04, False):
                ...
                arr.add(big_tmp[pos + 1])        
            if midJudge(big_tmp, gapStat, 0.06, True):
                for i in range(len(big_tmp) - 2, -1, -1):
                    if big_gap_tmp[i] > 0.06 and big_gap_tmp[i] - big_gap_tmp[i - 1] < 0.001:
                        arr.add(big_tmp[i])
                        break
            if midJudge(big_tmp, gapStat, 0.07, False):
                pos = len(big_tmp) - gapStat[0.07] - 1
            ...
            if midJudge(big_tmp, gapStat, 0.02, True):
                ...
                arr.add(big_tmp[pos + 1])
                for i in range(pos + 2, len(big_tmp)):
                    if big_gap_tmp[i] - big_gap_tmp[pos + 1] < 0.00001:
                        arr.add(big_tmp[i])
                arr.add(big_tmp[pos])
                if big_gap_tmp[pos] > 0.019 and pos - 1 > -1:
                    arr.add(big_tmp[pos - 1])
                for i in range(pos - 1, -1, -1):
                    if big_gap_tmp[pos] - big_gap_tmp[i] < 0.0001:
                        arr.add(big_tmp[i])
            if midJudge(big_tmp, gapStat, 0.03, False):
                pos = len(big_tmp) - gapStat[0.03]
                if big_gap_tmp[pos] < 0.031:
                    arr.add(big_tmp[pos])
                    arr.add(big_tmp[pos + 1])
                if big_gap_tmp[pos - 1] >= 0.028 and pos - 2 > -1 and big_gap_tmp[pos - 2] < 0.028:
                    arr.add(big_tmp[pos - 2])
                else:
                    arr.add(big_tmp[pos - 1])
                    for i in range(pos - 2, -1, -1):
                ...
                if big_gap_tmp[pos - 1] > 0.0285:
                    ...
                    for i in range(pos - 2, -1, -1):
                        if big_gap_tmp[pos - 1] - big_gap_tmp[i] < 0.00001:
                            arr.add(big_tmp[i])
                if big_gap_tmp[pos] < 0.0317 and big_gap_tmp[pos + 1] > 0.034:
                    arr.add(big_tmp[pos + 1])
                elif big_gap_tmp[pos + 1] == big_gap_tmp[pos]:
                    arr.add(big_tmp[pos + 1])
                    arr.add(big_tmp[pos])
            if len(big_tmp) == gapStat[0.02]:
        ...
        if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) >= len(small_near_tmp):
            ...
            if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                if noConsecutiveGap(big_near_gap_tmp):
                    if big_near_gap_tmp[0] < 0.005 and big_near_gap_tmp[1] > 0.005:
                        arr.add(big_near_tmp[1])
                    if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[1] > 0.003:
                        arr.add(big_near_tmp[1])
                    if big_near_gap_tmp[0] < 0.0015:
                        arr.add(big_near_tmp[0])
                pos = len(big_near_tmp) - nearGapStat[0.01]
                if big_near_gap_tmp[pos] < 0.012 and pos + 1 < len(big_near_tmp):
                    arr.add(big_near_tmp[pos + 1])
                else:
                    arr.add(big_near_tmp[pos])
                    for i in range(pos + 1, len(big_near_tmp)):
            ...
            if nearGapStat[0.01] == 0:
                ...
                findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
            if nearGapStat[0.01] == 0 or nearGapStat[0.01] == len(big_near_tmp):
                if len(big_near_tmp) > 2 and big_near_gap_tmp[0] == big_near_gap_tmp[1] and big_near_gap_tmp[1] == big_near_gap_tmp[2]:
                    arr.add(big_near_tmp[0])
                    arr.add(big_near_tmp[1])
                    arr.add(big_near_tmp[2])
                if noConsecutiveGap(big_near_gap_tmp):
                    if big_near_gap_tmp[-1] > 0.008 and big_near_gap_tmp[-2] < 0.0052:
                        arr.add(big_near_tmp[-2])
        ...
        if len(small_near_tmp) >= len(big_near_tmp):
            ...
            if nearGapStat[-0.01] == 0:
                if small_near_gap_tmp[-1] > -0.0005:
                    arr.add(small_near_tmp[-1])
                if noConsecutiveGap(small_near_gap_tmp):
                    if small_near_gap_tmp[-2] < -0.0075:
                        arr.add(small_near_tmp[-1])
                if small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                    for v in small_near_tmp:
                        arr.add(v)
        ...
        if len(small_tmp) - len(big_near_tmp) >= -1:
            ...
            if len(small_tmp) == 2:
                if gapStat[-0.02] == 0:
                    arr.add(small_tmp[1])
                if gapStat[-0.03] == 0:
                    if small_gap_tmp[0] < -0.026:
                        arr.add(small_tmp[1])
            if len(small_tmp) > 2:
                if len(small_tmp) == gapStat[-0.02]:
                    if noConsecutiveGap(small_gap_tmp):
                        if small_gap_tmp[0] <= -0.028:
                            arr.add(small_tmp[1])
            if midJudge(small_tmp, gapStat, -0.03, False):
                pos = gapStat[-0.03]
                gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=False)
        if len(small_tmp) == 2:
            if small_gap_tmp[0] > -0.0201:
                arr.add(small_tmp[0])
            if gapStat[-0.02] == 1:
                arr.add(small_tmp[0])
            if gapStat[-0.03] == 2:
            ...
            if gapStat[-0.02] == 0:
                arr.add(small_tmp[0])
                if small_gap_tmp[1] - small_gap_tmp[0] < 0.001:
                    arr.add(small_tmp[1])
        if len(small_tmp) == 1:
            if gapStat[-0.02] == 0:
                arr.add(small_tmp[0])
        flag = ':参差' if maxConsecutiveCount['big'] < 4 * 1.5 else ':连续'
        print('不是大的就是接近的(大的偏多)' + flag + restExcBig(seq, 1.5))
    elif count['big'] >= int(5 * 1.5) and count['small'] >= 2 * 1.5:
        if len(big_tmp) == gapStat[0.02]:
            if big_gap_tmp[0] < 0.025 and big_gap_tmp[1] > 0.025:
                arr.add(big_tmp[1])
            if big_gap_tmp[0] < 0.0205:
                arr.add(big_tmp[0])
                for i in range(1, len(big_tmp)):
                    if big_gap_tmp[i] - big_gap_tmp[0] < 0.0001:
                        arr.add(big_tmp[i])
        if midJudge(big_tmp, gapStat, 0.04, False):
            pos = len(big_tmp) - gapStat[0.04] - 1
            arr.add(big_tmp[pos])
        ...
        if len(small_tmp) >= len(big_near_tmp):
            ...
            if midJudge(small_tmp, gapStat, -0.02, True):
                pos = gapStat[-0.02] - 1
                if small_gap_tmp[pos] > -0.021:
                    arr.add(small_tmp[pos - 1])
                if small_gap_tmp[0] < -0.0283:
                    arr.add(small_tmp[1])
                arr.add(small_tmp[pos + 1])
                for i in range(pos + 2, len(small_tmp)):
                    if small_gap_tmp[i] - small_gap_tmp[pos] < 0.0001:
            ...
            if midJudge(small_tmp, gapStat, -0.02, False):
                pos = gapStat[-0.02] - 1
                gapAndConsecutive(arr, small_tmp, small_tmp, pos, left=False)
                if small_gap_tmp[pos] > -0.0234:
                    if pos - 1 > -1:
                        arr.add(small_tmp[pos - 1])
        flag = ':参差' if maxConsecutiveCount['big'] < 4 * 1.5 else ':连续'
        print('不是大的就是小的(大的偏多)' + flag + restExcBig(seq, 1.5))
    #大:接近:小很均衡
    elif count['big'] <= 4 * 1.5 and count['near'] <= 4 * 1.5 and count['small'] <= 4 * 1.5:
        if len(seq['small']) + len(seq['smallNear']) > len(seq['big']) + len(seq['bigNear']):
            if len(big_near_tmp) - len(small_tmp) >= -2 and len(big_near_tmp) >= len(small_near_tmp):
                if nearGapStat[0.01] == 0:
                    if noConsecutiveGap(big_near_gap_tmp):
                        if big_near_gap_tmp[0] < 0.001:
                            arr.add(big_near_tmp[1])
            ...
            if len(small_tmp) >= len(big_tmp) and len(small_tmp) >= len(big_near_tmp):
                ...
                if midJudge(small_tmp, gapStat, -0.03, False):
                    ...
                    for i in range(pos - 1, -1, -1):
                        if small_gap_tmp[pos] - small_gap_tmp[i] < 0.0001:
                            arr.add(small_tmp[i])
                    if small_gap_tmp[pos] > -0.0311 and pos - 1 > -1:
                        arr.add(small_tmp[pos - 1])
                    if small_gap_tmp[pos + 1] <= -0.029:
                        if small_gap_tmp[pos + 2] > -0.027 or pos + 3 < len(small_tmp) and small_gap_tmp[pos + 3] > -0.021:
                            arr.add(small_tmp[pos + 2])
                    gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos + 1, left=False)
                if len(small_tmp) == gapStat[-0.02]:
                    if midJudge(small_tmp, gapStat, -0.03, False):
            ...
            if len(big_tmp) - len(small_tmp) >= -2 and len(big_tmp) >= len(small_near_tmp):
                ...
                if midJudge(big_tmp, gapStat, 0.03, False):
                    pos = len(big_tmp) - gapStat[0.03] - 1
                    arr.add(big_tmp[pos])
                    if big_gap_tmp[-1] > 0.037 and big_gap_tmp[-1] < 0.04:
                        arr.add(big_tmp[-1])
                    arr.add(big_tmp[pos + 1])
                    for i in range(pos + 2, len(big_tmp)):
                        if big_gap_tmp[i] - big_gap_tmp[pos + 1] < 0.001:
                            arr.add(big_tmp[i])
                if midJudge(big_tmp, gapStat, 0.04, False):
                    pos = len(big_tmp) - gapStat[0.04]
                    arr.add(big_tmp[pos])
                    for i in range(pos + 1, len(big_tmp)):
                        if big_gap_tmp[i] - big_gap_tmp[pos] < 0.001:
                            arr.add(big_tmp[i])
            elif len(big_tmp) >= 3:
                if midJudge(big_tmp, gapStat, 0.02, False):
                    pos = len(big_tmp) - gapStat[0.02] - 1
            ...
            if len(small_near_tmp) - len(big_tmp) >= -1 and len(small_near_tmp) >= len(big_near_tmp):
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    arr.add(small_near_tmp[pos])
                    if noConsecutiveGap(small_near_gap_tmp):
                        if small_near_gap_tmp[-1] > -0.003 and small_near_gap_tmp[-2] < -0.005:
                            arr.add(small_near_tmp[-1])
                if nearGapStat[-0.01] == 0:
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=False, consecutive_at_left=False, choose_single=True)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=False, consecutive_at_left=True, choose_single=False)
            print('很均衡时小的偏多' + extraSmall(seq, 1.5))
        elif len(seq['small']) + len(seq['smallNear']) < len(seq['big']) + len(seq['bigNear']):
            if len(big_near_tmp) - len(small_tmp) >= -2 and len(big_near_tmp) - len(small_near_tmp) >= -2:
                if len(big_near_tmp) == 2:
                    if nearGapStat[0.01] == 1:
                        arr.add(big_near_tmp[1])
                        arr.add(big_near_tmp[0]) 
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    ...
                    for i in range(pos - 2, -1, -1):
                        if big_near_gap_tmp[pos - 1] - big_near_gap_tmp[i] < 0.0001:
                            arr.add(big_near_tmp[i])
                    if big_near_gap_tmp[pos] < 0.012 and pos + 1 < len(big_near_tmp) and big_near_gap_tmp[pos + 1] > 0.012:
                        arr.add(big_near_tmp[pos + 1])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                    pos = len(big_near_tmp) - nearGapStat[0.01]
            ...
            if len(big_tmp) - len(small_tmp) >= -1 and len(big_tmp) >= len(small_near_tmp):
                ...
                if midJudge(big_tmp, gapStat, 0.03, True):
                    if gapStat[0.04] == 0:
                        if big_gap_tmp[-1] > 0.0391:
                            arr.add(big_tmp[-1])
                            for i in range(len(big_tmp) - 2, -1, -1):
                                if big_gap_tmp[-1] - big_gap_tmp[i] < 0.00001:
                                    arr.add(big_tmp[i])
                if midJudge(big_tmp, gapStat, 0.03, False):
                    pos = len(big_tmp) - gapStat[0.03] - 1
                    arr.add(big_tmp[pos])
                ...
                if midJudge(big_tmp, gapStat, 0.02, True):
                    ...
                    for i in range(pos - 1, -1, -1):
                        if big_gap_tmp[pos] - big_gap_tmp[i] < 0.0001:
                            arr.add(big_tmp[i])
                    arr.add(big_tmp[pos + 1])
                    if big_gap_tmp[1] > 0.018 and big_gap_tmp[0] < 0.016:
                        arr.add(big_tmp[0])
                    if big_gap_tmp[pos + 1] < 0.0215:
                        arr.add(big_tmp[pos + 2])
            if len(small_tmp) - len(big_tmp) >= -2 and len(small_tmp) - len(big_near_tmp) >= -2:
                if midJudge(small_tmp, gapStat, -0.02, True):
                    pos = gapStat[-0.02]
                    gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=False)
                    if small_gap_tmp[pos] < -0.019 and pos + 1 < len(small_tmp):
                        arr.add(small_tmp[pos])
                        arr.add(small_tmp[pos + 1])
                    else:
                        arr.add(small_tmp[pos])
                ...
                if midJudge(small_tmp, gapStat, -0.04, False):
                    pos = gapStat[-0.04] - 1
                    if small_gap_tmp[pos] > -0.0429:
                        if pos - 1 > -1:
                            arr.add(small_tmp[pos - 1])
                    if small_gap_tmp[pos] > -0.0441 and pos - 1 > -1 and small_gap_tmp[pos - 1] < -0.044:
                        arr.add(small_tmp[pos - 1])
                    gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=True)
                if midJudge(small_tmp, gapStat, -0.03, False):
                    pos = gapStat[-0.03] - 1
                ...
                if small_gap_tmp[0] < -0.0275 and small_gap_tmp[0] > -0.031:
                    arr.add(small_tmp[0])
                if midJudge(small_tmp, gapStat, -0.02, False):
                    pos = gapStat[-0.02]
                    gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=False)
            ...
            if len(small_near_tmp) - len(big_near_tmp) >= -1:
                ...
                if len(small_near_tmp) == 2:
                    if small_near_gap_tmp[0] < -0.005 and small_near_gap_tmp[1] > -0.003:
                        arr.add(small_near_tmp[0])
                    if small_near_gap_tmp[0] < -0.009:
                        arr.add(small_near_tmp[1])
                if nearGapStat[-0.01] == len(small_near_tmp):
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=True)
            print('很均衡时大的偏多' + extraBig(seq, 1.5))
        else:
            print('非常均衡')
    elif count['small'] >= 7 * 1.5:
        if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) >= len(big_near_tmp):
            if len(small_near_tmp) == 2:
                if small_near_gap_tmp[0] < -0.0067 and small_near_gap_tmp[1] > -0.0022:
                    arr.add(small_near_tmp[0])
        if len(small_tmp) == gapStat[-0.02]:
            if small_gap_tmp[-1] > -0.021:
                arr.add(small_tmp[-1])
        if len(small_tmp) == gapStat[-0.05]:
            if small_gap_tmp[-1] < -0.05 and small_gap_tmp[-1] > -0.051:
                arr.add(small_tmp[-1])
        if len(small_tmp) == gapStat[-0.04]:
            if small_gap_tmp[-2] > -0.05 and small_gap_tmp[-2] < -0.049:
                arr.add(small_tmp[-1])
        if midJudge(small_tmp, gapStat, -0.05, False):
            pos = gapStat[-0.05] - 1
            arr.add(small_tmp[pos])
        ...
        if midJudge(small_tmp, gapStat, -0.03, False):
            pos = gapStat[-0.03]
            if small_gap_tmp[pos] < -0.0285:
                if pos + 1 < len(small_tmp):
                    arr.add(small_tmp[pos + 1])
        if midJudge(small_tmp, gapStat, -0.03, True):
            pos = gapStat[-0.03] - 1
            if small_gap_tmp[pos] > -0.033:
                arr.add(small_tmp[pos - 1])
            else:
                arr.add(small_tmp[pos])
            arr.add(small_tmp[pos + 1])
        ...
        if midJudge(small_tmp, gapStat, -0.08, False):
            ...
            gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos - 1, left=True)
        if midJudge(small_tmp, gapStat, -0.06, False):
            if small_gap_tmp[0] > -0.07 and small_gap_tmp[0] < -0.068:
                arr.add(small_tmp[1])
            pos = gapStat[-0.06]
            arr.add(small_tmp[pos])
            for i in range(pos + 1, len(small_tmp)):
        ...
        if midJudge(small_tmp, gapStat, -0.07, False):
            ...
            pos = gapStat[-0.07]
            arr.add(small_tmp[pos])
            if small_gap_tmp[pos] < -0.068 and small_gap_tmp[pos + 1] > -0.062:
                arr.add(small_tmp[pos + 1])
            arr.add(small_tmp[pos - 1])
            for i in range(pos - 2, -1, -1):
                if small_gap_tmp[pos - 1] - small_gap_tmp[i] < 0.0001:
        ...
        print('连续小' + restExcSmall(seq, 1.5))
    else:
        print('不对呀', data_len)
