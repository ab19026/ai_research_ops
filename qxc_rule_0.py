...
def process(count, maxConsecutiveCount, seq, nearGapStat, gapStat, prob_cand_arr, pred, avg_window, data_len, good_count, range_stat, name, label, label_dim):
        ...
        if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) - len(small_near_tmp) >= -1:
            ...
            if nearGapStat[0.01] == 0:
                if noConsecutiveGap(big_near_gap_tmp):
                    if len(big_near_gap_tmp) > 0 and big_near_gap_tmp[-1] > 0.009:
            ...
            if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                ...
                for i in range(pos + 1, len(big_near_gap_tmp)):
        ...
        if len(seq['small']) + len(seq['smallNear']) >= 7:
            if len(big_tmp) - len(small_tmp) >= -1:
                if len(big_tmp) == 2:
                    if gapStat[0.04] == 1:
                        arr.add(big_tmp[1])
            if len(big_near_tmp) == 1 and nearGapStat[0.01] == 0:
                arr.add(big_near_tmp[0])
            ...
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) - len(big_near_tmp) >= -1:
                if midJudge(small_near_tmp, nearGapStat, -0.01, True):
                    ...
                    if small_near_gap_tmp[pos + 1] < -0.0089:
            ...
            if len(small_tmp) - len(big_tmp) >= -1 and len(small_tmp) - len(big_near_tmp) >= -1:
                ...
                if midJudge(small_tmp, gapStat, -0.03, False):
                    if small_gap_tmp[1] > -0.032 and small_gap_tmp[0] < -0.0375:
                        arr.add(small_tmp[0])
                    pos = gapStat[-0.03] - 1
                    arr.add(small_tmp[pos])
                    for i in range(pos - 1, -1, -1):
            ...
            if len(big_tmp) >= len(small_tmp) and len(big_tmp) >= len(small_near_tmp):
                if len(big_tmp) == 2:
                    if gapStat[0.05] == 1:
                        arr.add(big_tmp[1])
                if big_gap_tmp[0] < 0.021 and big_gap_tmp[1] > 0.025:
                    arr.add(big_tmp[0])
                if midJudge(big_tmp, gapStat, 0.03, True):
            ...
            if len(small_tmp) == 1:
                if gapStat[-0.02] == 0:
                    arr.add(small_tmp[0])
                if small_gap_tmp[0] > -0.021 and small_gap_tmp[0] < -0.02:
                    arr.add(small_tmp[0])
            ...
            if len(big_near_tmp) - len(small_tmp) >= -1:
                if len(big_near_tmp) == 1:
                    if big_near_gap_tmp[0] < 0.008:
                        arr.add(big_near_tmp[0])
                if len(big_near_tmp) == 2:
                    if nearGapStat[0.01] == 0:
                        if nearGapStat[0.01] == 0:
                            if big_near_gap_tmp[0] < 0.0067 and big_near_gap_tmp[1] > 0.0085:
                                arr.add(big_near_tmp[0])
                        if big_near_gap_tmp[0] == big_near_gap_tmp[1]:
                            arr.add(big_near_tmp[0])
                            arr.add(big_near_tmp[1])
                        if big_near_gap_tmp[0] <= 0.0055 and big_near_gap_tmp[1] > 0.008:
                            arr.add(big_near_tmp[0])
                            arr.add(big_near_tmp[1])
                        ...
                        if big_near_gap_tmp[1] > 0.007 and big_near_gap_tmp[0] < 0.004:
                            arr.add(big_near_tmp[1])
                        if big_near_gap_tmp[0] < 0.0017 and big_near_gap_tmp[1] > 0.005:
                            arr.add(big_near_tmp[0])
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
            ...
            if len(big_tmp) - len(small_tmp) >= -1 and len(big_tmp) >= len(small_near_tmp):
                if len(big_tmp) == 2:
                    if gapStat[0.05] == 0:
                        if big_gap_tmp[1] > 0.045 and big_gap_tmp[0] < 0.0432:
                            arr.add(big_tmp[1])
                    if gapStat[0.02] == 1:
                        arr.add(big_tmp[0])
                    if gapStat[0.03] == 2:
            ...
            if len(small_tmp) - len(big_tmp) >= -1 and len(small_tmp) - len(big_near_tmp) >= -1:
                ...
                if len(small_tmp) == gapStat[-0.02]:
                    if small_gap_tmp[0] < -0.029:
                        arr.add(small_tmp[1])
                    if gapStat[-0.03] == 0:
                        if len(small_tmp) > 2 and small_gap_tmp[0] == small_gap_tmp[1] and small_gap_tmp[1] == small_gap_tmp[2]:
                            arr.add(small_tmp[0])
                            arr.add(small_tmp[1])
                            arr.add(small_tmp[2])
            if len(big_near_tmp) - len(small_near_tmp) >= -1 and len(big_near_tmp) - len(small_tmp) >= -1:
                ...
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    ...
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=False)
                    if len(big_near_tmp) > 3:
                        if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[2] > 0.008:
                            arr.add(big_near_tmp[1])
                if nearGapStat[0.01] == 0:
            ...
            if len(small_near_tmp) - len(big_near_tmp) >= -1 and len(small_near_tmp) - len(big_tmp) >= -1:
                if small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                    for v in small_near_tmp:
                        arr.add(v)
                if nearGapStat[-0.01] == 0:
                    if small_near_gap_tmp[0] < -0.0095:
                        arr.add(small_near_tmp[1])
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
            if len(small_near_tmp) == 1:
                if nearGapStat[-0.01] == 0:
            ...
            if len(small_near_tmp) - len(big_tmp) >= -1 and len(small_near_tmp) - len(big_near_tmp) >= -1:
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    if noConsecutiveGap(small_near_gap_tmp):
                        pos = nearGapStat[-0.01]
                        if small_near_gap_tmp[pos] < -0.0085:
            ...
            if len(big_tmp) - len(small_tmp) >= -1 and len(big_tmp) - len(small_near_tmp) >= -1:
                ...
                if midJudge(big_tmp, gapStat, 0.05, False):
                    ...
                    for i in range(pos + 1, len(big_tmp)):
                        if big_gap_tmp[i] - big_gap_tmp[pos] < 0.0001:
                            arr.add(big_tmp[i])
            if len(small_tmp) - len(big_tmp) >= -2 and len(small_tmp) - len(big_near_tmp) >= -1:  
                if midJudge(small_tmp, gapStat, -0.03, True):
                    pos = gapStat[-0.03] - 1
                    arr.add(small_tmp[pos])
                    for i in range(pos - 1, -1, -1):
                        if small_gap_tmp[pos] - small_gap_tmp[i] < 0.00001:
                            arr.add(small_tmp[i])
                if midJudge(small_tmp, gapStat, -0.02, True):
                    if small_gap_tmp[0] < -0.024 and small_gap_tmp[0] > -0.0255 and small_gap_tmp[1] > -0.022:
                        arr.add(small_tmp[0])
                    pos = gapStat[-0.02] - 1
                    arr.add(small_tmp[pos])
                    for i in range(pos - 1, -1, -1):
                    ...
                    if gapStat[-0.03] == 0:
                        if len(small_tmp) > 2:
                            if small_gap_tmp[0] == small_gap_tmp[1] and small_gap_tmp[1] == small_gap_tmp[2]:
                                ...
                                arr.add(small_tmp[2])
                if len(small_tmp) == 2:
                    if small_gap_tmp[0] < -0.028 and small_gap_tmp[1] > -0.0203:
                        arr.add(small_tmp[0])
                    if gapStat[-0.02] == 1:
                        arr.add(small_tmp[1])
                if len(small_tmp) == 1:
    ...
    #连续接近
        ...
        if len(seq['small']) + len(seq['smallNear']) >= 7:
            ...
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) >= len(small_near_tmp):
                ...
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    ...
                    for i in range(pos - 1, -1, -1):
                        if big_near_gap_tmp[pos] - big_near_gap_tmp[i] < 0.0001:
                            arr.add(big_near_tmp[i])
                    arr.add(big_near_tmp[pos + 1])
                    for i in range(pos + 2, len(big_near_gap_tmp)):
                ...
                if nearGapStat[0.01] == 0:
                    ...
                    if big_near_gap_tmp[-1] > 0.008 and big_near_gap_tmp[-2] < 0.005:
                        arr.add(big_near_tmp[-1])
            if len(big_tmp) >= len(small_tmp) and len(big_tmp) - len(small_near_tmp) >= -1:
                ...
                if len(big_tmp) == 2:
                    if gapStat[0.04] == 1:
                        arr.add(big_tmp[0])
            if len(small_near_tmp) - len(big_near_tmp) >= -2 and len(small_near_tmp) >= len(big_tmp):
                if nearGapStat[-0.01] == 1:
                    arr.add(small_near_tmp[1])
                if nearGapStat[-0.01] == len(small_near_tmp):
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=False, consecutive_at_left=True, choose_single=False)
            ...
            if len(small_near_tmp) - len(big_tmp) >= -1:
                if len(small_near_tmp) <= 2:
                    for bi in small_near_tmp:
                        if prob_cand_arr[bi] - pred > -0.01:
                            arr.add(bi)
                else:
                    if midJudge(small_near_tmp, nearGapStat, -0.01, True):
                        pos = nearGapStat[-0.01] - 1
            ...
            if len(small_near_tmp) <= 2:
                if nearGapStat[-0.01] == 1:
                    arr.add(small_near_tmp[0])
                if len(small_near_tmp) == 2 and small_near_gap_tmp[0] == small_near_gap_tmp[1]:
                    arr.add(small_near_tmp[0])
                    arr.add(small_near_tmp[1])
            ...
            if len(small_near_tmp) > len(big_tmp) and len(small_near_tmp) >= len(big_near_tmp):
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    arr.add(small_near_tmp[pos])
                    for i in range(pos + 1, len(small_near_tmp)):
                        if small_near_gap_tmp[i] - small_near_gap_tmp[pos] < 0.001:
                            arr.add(small_near_tmp[i])
            ...
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) - len(small_near_tmp) >= -1:
                ...
                if len(big_near_tmp) == 2:
                    if nearGapStat[0.01] == 1:
                        arr.add(big_near_tmp[1])
                    if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[1] > 0.007:
                        arr.add(big_near_tmp[0])
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) >= len(big_near_tmp):
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    if small_near_gap_tmp[-1] > -0.002 and small_near_gap_tmp[-2] <= -0.0048:
                        arr.add(small_near_tmp[-1])
                    pos = nearGapStat[-0.01]
                    arr.add(small_near_tmp[pos])
                    for i in range(pos + 1, len(small_near_tmp)):
        ...
        if midJudge(small_tmp, gapStat, -0.04, True):
            ...
            for i in range(pos - 1, -1, -1):
                if small_gap_tmp[pos] - small_gap_tmp[i] < 0.0001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[pos] > -0.0421 and pos - 2 > -1 and small_gap_tmp[pos - 2] < -0.047:
                arr.add(small_tmp[pos - 1])
        ...
        if midJudge(small_tmp, gapStat, -0.03, False):
            ...
            for i in range(pos - 2, -1, -1):
                if small_gap_tmp[pos - 1] - small_gap_tmp[i] < 0.0001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[pos - 1] > -0.0316:
                if pos - 2 > -1:
                    arr.add(small_tmp[pos - 2])
        if midJudge(small_tmp, gapStat, -0.03, True):
            if small_gap_tmp[0] < -0.0371 and small_gap_tmp[1] > -0.0354:
                arr.add(small_tmp[0])
            pos = gapStat[-0.03]
            arr.add(small_tmp[pos])
            for i in range(pos + 1, len(small_tmp)):
            ...
            for i in range(pos - 2, -1, -1):
                if small_gap_tmp[pos - 1] - small_gap_tmp[i] < 0.0001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[-1] > -0.023 and small_gap_tmp[-2] < -0.025 or small_near_gap_tmp[-1] > -0.021:
                arr.add(small_tmp[-1])
        if midJudge(small_tmp, gapStat, -0.02, True):
            pos = gapStat[-0.02]
            arr.add(small_tmp[pos])
        ...
        if len(small_tmp) == gapStat[-0.02]:
            ...
            if small_gap_tmp[0] == small_gap_tmp[-1]:
                for i in range(len(small_tmp)):
                    arr.add(small_tmp[i])
        if len(big_tmp) - len(small_near_tmp) >= -1:
            if len(big_tmp) == 1:
                if big_gap_tmp[0] < 0.026:
                    arr.add(big_tmp[0])
            if len(big_tmp) == 2:
                if gapStat[0.03] == 1:
                    arr.add(big_tmp[1])
                if gapStat[0.03] == 0:
                    if big_gap_tmp[1] >= 0.0275:
                        arr.add(big_tmp[1])
                    if big_gap_tmp[0] < 0.021:
                        if big_gap_tmp[0] == big_gap_tmp[1]:
                            arr.add(big_tmp[0])
        ...
        if len(big_tmp) >= len(small_tmp) and len(big_tmp) >= len(small_near_tmp):
            if midJudge(big_tmp, gapStat, 0.02, True):
                ...
                for i in range(pos - 1, -1, -1):
                    if big_gap_tmp[pos] - big_gap_tmp[i] < 0.0001:
                        arr.add(big_tmp[i])
                if big_gap_tmp[0] > 0.015 and big_gap_tmp[0] < 0.0155 and big_gap_tmp[1] > 0.018:
                    arr.add(big_tmp[0])
            if midJudge(big_tmp, gapStat, 0.03, False):
                pos = len(big_tmp) - gapStat[0.03] - 1
                gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos, left=True)
            ...
            if midJudge(big_tmp, gapStat, 0.05, False):
                ...
                arr.add(big_tmp[pos - 1])
                for i in range(pos - 2, -1, -1):
                    if big_gap_tmp[pos - 1] - big_gap_tmp[i] < 0.001:
                        arr.add(big_tmp[i])
            if midJudge(big_tmp, gapStat, 0.06, False):
                pos = len(big_tmp) - gapStat[0.06]
                arr.add(big_tmp[pos])
        ...
        if len(big_near_tmp) >= len(small_near_tmp):
            ...
            if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                ...
                arr.add(big_near_tmp[pos])
                for i in range(pos + 1, len(big_near_gap_tmp)):
                    if big_near_gap_tmp[i] - big_near_gap_tmp[pos] < 0.001:
                        arr.add(big_near_tmp[i])
    ...
    #大:接近:小很均衡
        ...
        if len(seq['small']) + len(seq['smallNear']) > len(seq['big']) + len(seq['bigNear']):
            if len(big_tmp) == 2:
                if big_gap_tmp[0] < 0.021 and big_gap_tmp[1] > 0.0285:
                    arr.add(big_tmp[1])
            if len(big_tmp) - len(small_tmp) >= -2 and len(big_tmp) - len(small_near_tmp) >= -2:
                if len(big_tmp) == 2:
                    if gapStat[0.02] == 2:
                        if gapStat[0.03] == 1:
                            arr.add(big_tmp[0])
                        if gapStat[0.03] == 0:
                            if big_gap_tmp[0] < 0.022 and big_gap_tmp[1] > 0.025:
                                arr.add(big_tmp[0])
                            if big_gap_tmp[0] >= 0.0248 and big_gap_tmp[1] >= 0.0265:
                                arr.add(big_tmp[1])
                    if gapStat[0.02] <= 1:
                        arr.add(big_tmp[0])
                    if gapStat[0.03] == 1 or gapStat[0.04] == 1:
                        arr.add(big_tmp[1])
                    if big_gap_tmp[1] > 0.037 and big_gap_tmp[0] < 0.035:
                        arr.add(big_tmp[1])
                if midJudge(big_tmp, gapStat, 0.04, True):
                    pos = len(big_tmp) - gapStat[0.04]
                    gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos, left=False)
                if midJudge(big_tmp, gapStat, 0.03, True):
                    if big_gap_tmp[-2] < 0.0324:
                        arr.add(big_tmp[-1])
                if midJudge(big_tmp, gapStat, 0.02, True):
                    pos = len(big_tmp) - gapStat[0.02]
                    arr.add(big_tmp[pos])
            ...
            if len(big_tmp) >= len(small_near_tmp) and len(big_tmp) - len(small_tmp) >= -1 and len(big_tmp) - len(big_near_tmp) >= -1:
                if midJudge(big_tmp, gapStat, 0.04, True):
                    if big_gap_tmp[-1] > 0.047 and big_gap_tmp[-2] < 0.044:
                        arr.add(big_tmp[-1])
                if midJudge(big_tmp, gapStat, 0.04, False):
                    pos = len(big_tmp) - gapStat[0.04] - 1
                    arr.add(big_tmp[pos])
                ...
                if midJudge(big_tmp, gapStat, 0.02, True):
                    ...
                    for i in range(pos - 1, -1, -1):
                        if big_gap_tmp[pos] - big_gap_tmp[i] < 0.0001:
                            arr.add(big_tmp[i])
                    if big_gap_tmp[0] < 0.016 and big_gap_tmp[1] > 0.017:
                        arr.add(big_tmp[0])
                if midJudge(big_tmp, gapStat, 0.03, True):
                    pos = len(big_tmp) - gapStat[0.03]
                    arr.add(big_tmp[pos])
                ...
                if midJudge(big_tmp, gapStat, 0.03, False):
                    ...
                    for i in range(pos - 1, -1, -1):
                        if big_gap_tmp[pos] - big_gap_tmp[i] < 0.0001:
                            arr.add(big_tmp[i])
            if len(small_tmp) - len(big_near_tmp) >= -1 and len(small_tmp) - len(big_tmp) >= -2:
                if gapStat[-0.02] == 0:
                    findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                ...
                if len(small_tmp) == gapStat[-0.02]:
                    if len(small_tmp) > 2:
                        if small_gap_tmp[-1] > -0.022 and small_gap_tmp[-3] < -0.027:
                            arr.add(small_tmp[-1])
                        findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=True)
            if len(small_near_tmp) == 1:
                if nearGapStat[-0.01] == 0:
                    arr.add(small_near_tmp[0])
            if len(big_near_tmp) - len(big_tmp) >= -1 and len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) > len(small_near_tmp):
                if nearGapStat[0.01] == 0:
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    pos = len(big_near_tmp) - nearGapStat[0.01] - 1
                    ...
                    for i in range(pos + 2, len(big_near_gap_tmp)):
                        if big_near_gap_tmp[i] - big_near_gap_tmp[pos + 1] < 0.0001:
                            arr.add(big_near_tmp[i])
                    if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[1] > 0.006:
                        arr.add(big_near_tmp[0])
            if len(small_near_tmp) >= len(big_near_tmp) and len(small_near_tmp) - len(big_tmp) >= -2:
                if len(small_near_tmp) == 2:
                    if small_near_gap_tmp[0] <= -0.009:
                        arr.add(small_near_tmp[1])
            ...
            if len(big_tmp) == len(small_tmp):
                ...
                if gapStat[0.03] == len(big_tmp):
                    if gapStat[0.04] == 0:
                        if len(big_tmp) > 2 and big_gap_tmp[0] == big_gap_tmp[1] and big_gap_tmp[1] == big_gap_tmp[2]:
                            arr.add(big_tmp[0])
                            arr.add(big_tmp[1])
                            arr.add(big_tmp[2])
                if len(small_near_tmp) == 2:
                ...
                if midJudge(small_tmp, gapStat, -0.03, False):
                    ...
                    arr.add(small_tmp[pos])
                    for i in range(pos - 1, -1, -1):
                        if small_gap_tmp[pos] - small_gap_tmp[i] < 0.001:
                            arr.add(small_tmp[i])
                ...
                if midJudge(small_tmp, gapStat, -0.02, True):
                    ...
                    for i in range(pos - 2, -1, -1):
                        if small_gap_tmp[pos - 1] - small_gap_tmp[i] < 0.0001:
                            arr.add(small_tmp[i])
                if midJudge(small_tmp, gapStat, -0.03, False):
                    if small_gap_tmp[1] > -0.033:
                        arr.add(small_tmp[0])
                    pos = gapStat[-0.03]
                    if pos + 1 < len(small_tmp) and small_gap_tmp[pos + 1] > -0.021:
                        arr.add(small_tmp[pos + 1])
                    else:
                        arr.add(small_tmp[pos])
                        for i in range(pos + 1, len(small_tmp)):
        ...
        if midJudge(small_tmp, gapStat, -0.03, False):
            if small_gap_tmp[0] < -0.038 and small_gap_tmp[2] > -0.0313:
                arr.add(small_tmp[1])
            pos = gapStat[-0.03] - 1
            arr.add(small_tmp[pos])
            for i in range(pos - 1, -1, -1):
        ...
        if midJudge(small_tmp, gapStat, -0.03, True):
            ...
            for i in range(pos - 2, -1, -1):
                if small_gap_tmp[pos - 1] - small_gap_tmp[i] < 0.0001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[pos - 1] > -0.034 and small_gap_tmp[pos - 3] < -0.038:
                arr.add(small_tmp[pos - 2])
        ...
        if midJudge(small_tmp, gapStat, -0.07, False):
            ...
            for i in range(pos - 1, -1, -1):
                if small_gap_tmp[pos] - small_gap_tmp[i] < 0.0001:
                    arr.add(small_tmp[i])
            findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=False, consecutive_at_left=True, choose_single=False)
            if small_gap_tmp[1] > -0.074 and small_gap_tmp[0] < -0.076:
                arr.add(small_tmp[0])
        if midJudge(small_tmp, gapStat, -0.06, True):
            pos = gapStat[-0.06] - 1
            arr.add(small_tmp[pos])
        ...
        if midJudge(small_tmp, gapStat, -0.05, True):
            ...
            arr.add(small_tmp[pos + 1])
            for i in range(pos + 2, len(small_tmp)):
                if small_gap_tmp[i] - small_gap_tmp[pos + 1] < 0.001:
                    arr.add(small_tmp[i])
        ...
        if midJudge(small_tmp, gapStat, -0.04, False):
            ...
            for i in range(pos + 2, len(small_tmp)):
                if small_gap_tmp[i] - small_gap_tmp[pos + 1] < 0.0001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[0] < -0.046:
                arr.add(small_tmp[0])
        if len(big_tmp) - len(small_near_tmp) >= -1:
            if len(big_tmp) == gapStat[0.02]:
                if gapStat[0.03] == 0:
                    if len(big_gap_tmp) > 0 and big_gap_tmp[-1] > 0.028:
                        arr.add(big_tmp[-1])
            if len(big_tmp) == 1:
                if gapStat[0.02] == 0:
                    arr.add(big_tmp[0])
        ...
        if len(small_near_tmp) >= len(big_near_tmp) and len(small_near_tmp) >= len(big_tmp):
            if len(small_near_tmp) == 2:
                if nearGapStat[-0.01] == 0:
                    if small_near_gap_tmp[1] > -0.0043 and small_near_gap_tmp[0] < -0.006:
                        arr.add(small_near_tmp[1])
        print('大部分小')
    elif count['small'] >= 5 and maxConsecutiveCount['small'] >= 4:
        if midJudge(small_tmp, gapStat, -0.05, False):
            ...
            for i in range(pos + 2, len(small_tmp)):
                if small_gap_tmp[i] - small_gap_tmp[pos + 1] < 0.001:
                    arr.add(small_tmp[i])
            if small_gap_tmp[pos] > -0.0527 and small_gap_tmp[pos - 1] < -0.054:
                arr.add(small_tmp[pos - 1])
        if midJudge(small_tmp, gapStat, -0.04, True):
            findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.05, reverse=False, consecutive_at_left=True, choose_single=False)
            pos = gapStat[-0.04]
        ...
        if len(big_tmp) - len(small_near_tmp) >= -1:
            ...
            if len(big_tmp) == 2:
                ...
                if gapStat[0.03] == 1:
                    arr.add(big_tmp[0])
                if gapStat[0.03] == 0:
                    if big_gap_tmp[1] >= 0.027:
                        arr.add(big_tmp[1])
                if gapStat[0.02] == 0:
                    if big_gap_tmp[0] < 0.016:
                        arr.add(big_tmp[0])
                    if big_gap_tmp[0] == big_gap_tmp[1]:
                        arr.add(big_tmp[0])
                        arr.add(big_tmp[1])
        if len(big_near_tmp) - len(small_near_tmp) >= -1:
            ...
            if len(big_near_tmp) == 2:
                if nearGapStat[0.01] == 1:
                    ...
                    arr.add(big_near_tmp[1])
                if nearGapStat[0.01] == 0:
                    if big_near_gap_tmp[0] < 0.0019 and big_near_gap_tmp[1] > 0.0045:
                        arr.add(big_near_tmp[1])
                    if big_near_gap_tmp[0] < 0.004 and big_near_gap_tmp[1] > 0.0065:
                        arr.add(big_near_tmp[1])
                    if big_near_gap_tmp[1] > 0.0088:
                        arr.add(big_near_tmp[1])
                if nearGapStat[0.01] == 2:
                    if big_near_gap_tmp[0] < 0.012 and big_near_gap_tmp[1] > 0.012:
                        arr.add(big_near_tmp[1])
            if nearGapStat[0.01] == 0:
                findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
        if len(small_near_tmp) - len(big_tmp) >= -1 and len(small_near_tmp) >= len(big_near_tmp):
            ...
            if len(small_near_tmp) == 2:
                if small_near_gap_tmp[0] == small_near_gap_tmp[1]:
                    arr.add(small_near_tmp[0])
                    arr.add(small_near_tmp[1])
                if small_near_gap_tmp[1] > -0.0022 and small_near_gap_tmp[0] < -0.006:
                    arr.add(small_near_tmp[1])
                if nearGapStat[-0.01] == 1:
                    arr.add(small_near_tmp[1])
                    arr.add(small_near_tmp[0])
                if nearGapStat[-0.01] == 0:
                    if small_near_gap_tmp[0] == small_near_gap_tmp[1]:
                        arr.add(small_near_tmp[0])
                        arr.add(small_near_tmp[1])
                    if small_near_gap_tmp[0] < -0.007 and small_near_gap_tmp[1] > -0.004:
                        arr.add(small_near_tmp[1])
        if len(big_tmp) == 1:
            pass
