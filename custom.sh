...
do
    ...
    if [[ ! $run =~ "67108864" ]]; then
        ...
        if [[ "$name" == qxc ]]; then
            nohup bash run.sh 0 250,1500,5 latest $avg_window 0.06 "$name" 1 EMA_STATE 0.1 &
            nohup bash run.sh 1 260,1500,5 latest $avg_window 0.03 "$name" 1 EMA_WINDOW 'auto' &
            nohup bash run.sh 2 270,1500,5 latest $avg_window 0.06 "$name" 1 EMA_STATE 0.1 &
            nohup bash run.sh 3 280,1500,5 latest $avg_window 0.06 "$name" 1 EMA_STATE 0.1 &
