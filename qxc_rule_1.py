...
def process(count, maxConsecutiveCount, seq, nearGapStat, gapStat, prob_cand_arr, pred, avg_window, data_len, good_count, range_stat, name, label, label_dim):
    ...
    if count['big'] >= 7:
        ...
        if midJudge(big_tmp, gapStat, 0.03, False):
            ...
            if big_gap_tmp[pos] > 0.029:
                if pos - 1 > -1:
                    arr.add(big_tmp[pos - 1])
            if big_gap_tmp[pos] > 0.027 and big_gap_tmp[pos - 1] < 0.026:
                arr.add(big_tmp[pos - 1])
        if midJudge(big_tmp, gapStat, 0.03, True):
            findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=True)
            if gapStat[0.04] == 0:
                findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
            pos = len(big_tmp) - gapStat[0.03] - 1
            gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos, left=True)
        ...
        if midJudge(big_tmp, gapStat, 0.04, False):
            if big_gap_tmp[-1] > 0.0465 and big_gap_tmp[-2] < 0.043:
                arr.add(big_tmp[-1])
        ...
        if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) >= len(small_near_tmp):
            ...
            if len(big_near_tmp) == 2 and nearGapStat[0.01] == 1:
        ...
        if len(seq['small']) + len(seq['smallNear']) >= 7:
            if len(small_near_tmp) - len(big_near_tmp) >= -1 and len(small_near_tmp) >= len(big_tmp):
                if midJudge(small_near_tmp, nearGapStat, -0.01, True):
                    ...
                    if small_near_gap_tmp[pos] < -0.0088:
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    if small_near_gap_tmp[-1] > -0.0025 and small_near_gap_tmp[-2] < -0.0045:
                        arr.add(small_near_tmp[-1])
                    ...
                    if noConsecutiveGap(small_near_gap_tmp):
                        if small_near_gap_tmp[-1] > -0.0023 and small_near_gap_tmp[-2] < -0.0038:
                            arr.add(small_near_tmp[-1])
            ...
            elif len(big_near_tmp) - len(small_tmp) >= -1:
            ...
            if len(big_tmp) >= len(small_tmp) and len(big_tmp) >= len(small_near_tmp):
                ...
                if midJudge(big_tmp, gapStat, 0.02, False):
                    ...
                    if big_gap_tmp[-1] > 0.028:
                        arr.add(big_tmp[-1])
                    if big_gap_tmp[0] < 0.0156 and big_gap_tmp[1] > 0.017:
                        arr.add(big_tmp[0])
                    pos = len(big_tmp) - gapStat[0.02] - 1
                    arr.add(big_tmp[pos])
                    for i in range(pos - 1, -1, -1):
                ...
                if len(big_tmp) == gapStat[0.02]:
                    if noConsecutiveGap(big_gap_tmp):
                        if big_gap_tmp[0] < 0.022:
                            arr.add(big_tmp[0])
                    if len(big_tmp) == 3:
                        if big_gap_tmp[0] < 0.021 and big_gap_tmp[2] > 0.025:
                            arr.add(big_tmp[1])
                    if gapStat[0.03] == 0:
                        findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
                        findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=True)
            ...
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) >= len(small_near_tmp):
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    if len(big_near_tmp) > 3:
                        if big_near_gap_tmp[0] < 0.0015 and big_near_gap_tmp[2] > 0.005:
                                arr.add(big_near_tmp[1])
                    for i in range(len(big_near_tmp)):
                        if big_near_gap_tmp[i] < 0.003 and i + 2 < len(big_near_tmp) and big_near_gap_tmp[i+2] > 0.0071 and big_near_gap_tmp[i+2] < 0.0099:
                            arr.add(big_near_tmp[i + 1])
                            break
                    if noConsecutiveGap(big_near_gap_tmp):
                        if big_near_gap_tmp[0] < 0.0012 and big_near_gap_tmp[2] > 0.0048 and big_near_gap_tmp[2] < 0.0056:
                            arr.add(big_near_tmp[1])
                    if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[1] > 0.004:
                        arr.add(big_near_tmp[0])
                    pos = len(big_near_tmp) - nearGapStat[0.01] - 1
                    if big_near_gap_tmp[pos] > 0.0087:
                    ...
                    if big_near_gap_tmp[pos + 1] < 0.011:
                    ...
                    if noConsecutiveGap(big_near_gap_tmp):
                        if len(big_near_tmp) > 2:
                            if big_near_gap_tmp[0] > 0.004 and big_near_gap_tmp[0] < 0.005 and big_near_gap_tmp[1] > 0.007:
                                arr.add(big_near_tmp[0])
                        if big_near_gap_tmp[0] < 0.0034 and big_near_gap_tmp[1] > 0.005:
                            arr.add(big_near_tmp[0])
                ...
                if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                    ...
                    for i in range(pos + 1, len(big_near_tmp)):
                        if big_near_gap_tmp[i] - big_near_gap_tmp[pos] < 0.0001:
                            arr.add(big_near_tmp[i])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
                    if big_near_gap_tmp[0] < 0.001 and big_near_gap_tmp[1] > 0.005:
                        arr.add(big_near_tmp[0])
                ...
                if nearGapStat[0.01] == 0:
                    ...
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=True)
                    if noConsecutiveGap(big_near_gap_tmp):
                        if len(big_near_gap_tmp) > 3:
                            if big_near_gap_tmp[0] < 0.003 and big_near_gap_tmp[2] > 0.0065:
                                if big_near_gap_tmp[1] > 0.004 and big_near_gap_tmp[1] < 0.0056:
                                    arr.add(big_near_tmp[1])
                        if big_near_gap_tmp[-1] > 0.008 and big_near_gap_tmp[-2] < 0.0057:
                            arr.add(big_near_tmp[-1])
                        if big_near_gap_tmp[-2] < 0.006 and big_near_gap_tmp[-1] > 0.007:
                            arr.add(big_near_tmp[-1])
            ...
            if len(small_near_tmp) == 2:
                ...
                if nearGapStat[-0.01] == 1:
                    arr.add(small_near_tmp[0])
                    if small_near_gap_tmp[0] > -0.0115:
                        arr.add(small_near_tmp[1])
                if nearGapStat[-0.01] == 0:
                    arr.add(small_near_tmp[0])
                    arr.add(small_near_tmp[1])
            ...
            if len(small_tmp) >= len(big_tmp) and len(small_tmp) - len(big_near_tmp) >= -1:
                ...
                if gapStat[-0.03] == 0:
                    for bi in small_tmp:
                        if prob_cand_arr[bi] - pred > -0.021:
                            arr.add(bi)
                if gapStat[-0.05] == 1:
                    arr.add(small_tmp[0])
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    for i in range(pos + 1, len(small_near_tmp)):
                        if small_near_gap_tmp[i] - small_near_gap_tmp[pos] < 0.0001:
                            arr.add(small_near_tmp[i])
                    if noConsecutiveGap(small_near_gap_tmp):
                        if small_near_gap_tmp[-1] > -0.002 and small_near_gap_tmp[-2] < -0.005:
                            arr.add(small_near_tmp[-1])
                if nearGapStat[-0.01] == 0:
            ...
            if len(big_tmp) >= len(small_tmp):
                if len(big_tmp) == 2:
                    if big_gap_tmp[0] < 0.02 and big_gap_tmp[0] > 0.019:
                        arr.add(big_tmp[0])
            ...
            if len(big_near_tmp) - len(small_near_tmp) >= -1 and len(big_near_tmp) >= len(small_tmp):
                if len(big_near_tmp) > 3 and big_near_gap_tmp[0] < 0.004 and big_near_gap_tmp[2] > 0.0085:
                    arr.add(big_near_tmp[1]) 
                if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                    pos = len(big_near_tmp) - nearGapStat[0.01] - 1
                    gapAndConsecutive(arr, big_near_tmp, big_near_gap_tmp, pos, left=True)
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    ...
                    for i in range(pos + 1, len(big_near_tmp)):
                        if big_near_gap_tmp[i] - big_near_gap_tmp[pos] < 0.0001:
                            arr.add(big_near_tmp[i])
                    if big_near_gap_tmp[pos - 1] > 0.009:
                        arr.add(big_near_tmp[pos - 2])
                ...
                if nearGapStat[0.01] == 0:
                    if big_near_gap_tmp[-1] > 0.009:
                        arr.add(big_near_tmp[-1])
                    if len(big_near_tmp) > 2:
                        if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[1] > 0.0035:
                            arr.add(big_near_tmp[0])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=True)
            ...
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) - len(small_near_tmp) >= -1:
                if nearGapStat[0.01] == 0:
                    if big_near_gap_tmp[-1] > 0.0062 and big_near_gap_tmp[-2] < 0.005:
                        arr.add(big_near_tmp[-1])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    if noConsecutiveGap(big_near_gap_tmp):
            ...
            if len(big_near_tmp) >= len(small_tmp):
                if len(big_near_tmp) == 2:
                    if big_near_gap_tmp[0] < 0.002 and big_near_gap_tmp[1] > 0.0045:
                        arr.add(big_near_tmp[1])
            if len(small_near_tmp) - len(big_near_tmp) >= -1 and len(small_near_tmp) >= len(big_tmp):
                if midJudge(small_near_tmp, nearGapStat, -0.01, True):
                    pos = nearGapStat[-0.01] - 1
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    pos = nearGapStat[-0.01] - 1
                    gapAndConsecutive(arr, small_near_tmp, small_near_gap_tmp, pos, left=True)
                    if small_near_gap_tmp[pos] > -0.0109:
                        if pos - 1 > -1:
                            arr.add(small_near_tmp[pos - 1])
                    if small_near_gap_tmp[pos + 1] < -0.008:
            ...
            if len(big_near_tmp) == len(small_near_tmp):
                if len(small_tmp) == 2:
                    if gapStat[-0.02] == 1:
                        arr.add(small_tmp[1])
                findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.3, reverse=True, consecutive_at_left=True, choose_single=False)
    ...
    #连续接近
        ...
        if len(seq['small']) + len(seq['smallNear']) >= 7:
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) >= len(big_near_tmp):
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    if small_near_gap_tmp[pos + 1] < -0.009:
                        arr.add(small_near_tmp[pos + 2])
                        for i in range(pos + 3, len(small_near_tmp)):
                            if small_near_gap_tmp[i] - small_near_gap_tmp[pos + 2] < 0.0001:
                                arr.add(small_near_tmp[i])
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=True)
                    if small_near_gap_tmp[-1] > -0.001:
                        arr.add(small_near_tmp[-1])
                if midJudge(small_near_tmp, nearGapStat, -0.01, True):
                    if small_near_gap_tmp[0] < -0.0145 and small_near_gap_tmp[1] > -0.0129:
                        arr.add(small_near_tmp[0])
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=True)
                    pos = nearGapStat[-0.01]
                    gapAndConsecutive(arr, small_near_tmp, small_near_gap_tmp, pos, left=False)
                    gapAndConsecutive(arr, small_near_tmp, small_near_gap_tmp, pos - 1, left=True)
                    if small_near_gap_tmp[-1] > -0.0035 and small_near_gap_tmp[-2] < -0.005:
                        arr.add(small_near_tmp[-1])
                if nearGapStat[-0.01] == 0:
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=True)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=True)
                    if small_near_gap_tmp[0] < -0.0092:
                        arr.add(small_near_tmp[0])
            if len(big_near_tmp) - len(small_tmp) >= -1:
                ...
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    ...
                    gapAndConsecutive(arr, big_near_tmp, big_near_gap_tmp, pos, left=True)
                    if noConsecutiveGap(big_near_gap_tmp):
                        if big_near_gap_tmp[0] < 0.004 and big_near_gap_tmp[1] > 0.006:
                            arr.add(big_near_tmp[0])
                if nearGapStat[0.01] == 0:
                    if noConsecutiveGap(big_near_gap_tmp):
                        if len(big_near_tmp) == 3:
                            if big_near_gap_tmp[0] < 0.0026 and big_near_gap_tmp[2] > 0.006:
                                arr.add(big_near_tmp[1])
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
            ...
            if len(small_tmp) >= len(big_tmp) and len(small_tmp) - len(big_near_tmp) >= -1:
                if midJudge(small_tmp, gapStat, -0.03, True):
                    ...
                    if small_gap_tmp[-1] > -0.023 and small_gap_tmp[-2] < -0.024:
                        arr.add(small_tmp[-1])
                if midJudge(small_tmp, gapStat, -0.05, False):
                    pos = gapStat[-0.05]
                    gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=False)
            ...
            if len(small_near_tmp) >= len(big_tmp):
                ...
                if len(small_near_tmp) == 2:
                    if nearGapStat[-0.01] == 1:
                        arr.add(small_near_tmp[1])
                    if small_near_gap_tmp[0] < -0.006 and small_near_gap_tmp[1] > -0.002:
                        arr.add(small_near_tmp[1])
                if len(small_near_tmp) > 1 and small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                    for v in small_near_tmp:
                        arr.add(v)
            ...
            if len(small_tmp) >= len(big_tmp) and len(small_tmp) - len(big_near_tmp) >= -1:
                if len(small_tmp) == 2:
                    if gapStat[-0.04] == 1:
                        if small_gap_tmp[0] < -0.048:
                            arr.add(small_tmp[0])
                        arr.add(small_tmp[1])
                    if small_gap_tmp[0] < -0.029 and small_gap_tmp[1] > -0.022:
                        arr.add(small_tmp[1])
                if midJudge(small_tmp, gapStat, -0.02, True):
                    pos = gapStat[-0.02] - 1
                    gapAndConsecutive(arr, small_tmp, small_gap_tmp, pos, left=True)
                    if small_gap_tmp[pos + 1] < -0.019:
                        if pos + 2 < len(small_tmp):
                            arr.add(small_tmp[pos + 2])
                    if small_gap_tmp[pos + 1] < -0.018:
                        if pos + 2 < len(small_tmp):
                            arr.add(small_tmp[pos + 2])
            ...
            if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) >= len(big_near_tmp):
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    pos = nearGapStat[-0.01] - 1
                    if pos == 0 or small_near_gap_tmp[pos] < -0.011:
                        arr.add(small_near_tmp[pos])
                    else:
                        if pos - 1 > -1:
                            arr.add(small_near_tmp[pos - 1])
                    arr.add(small_near_tmp[pos + 1])
                    for i in range(pos + 2, len(small_near_tmp)):
                        if small_near_gap_tmp[i] - small_near_gap_tmp[pos + 1] < 0.001:
                            arr.add(small_near_tmp[i])
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
                    if small_near_gap_tmp[pos + 1] < -0.009:
                        if pos + 2 < len(small_near_tmp):
                            arr.add(small_near_tmp[pos + 2])
                if nearGapStat[-0.01] == 0:
                    if small_near_gap_tmp[0] < -0.009:
                        arr.add(small_near_tmp[0])
                    if noConsecutiveGap(small_near_gap_tmp):
                        if len(small_near_tmp) > 2:
                            if small_near_gap_tmp[-1] > -0.002 and small_near_gap_tmp[-3] < -0.006:
                                arr.add(small_near_tmp[-2])
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.03, reverse=True, consecutive_at_left=False, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) - len(small_near_tmp) >= -1:
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    pos = len(big_near_tmp) - nearGapStat[0.01] - 1
                ...
                if nearGapStat[0.01] == 0:
                    if noConsecutiveGap(big_near_gap_tmp):
                        if big_near_gap_tmp[-1] > 0.007 and big_near_gap_tmp[-2] < 0.006:
                            arr.add(big_near_tmp[-1])
                if nearGapStat[0.01] == len(big_near_tmp):
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) >= len(big_tmp):
                if midJudge(big_near_tmp, nearGapStat, 0.01, True):
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    pos = len(big_near_tmp) - nearGapStat[0.01] - 1
            ...
            if len(big_near_tmp) > len(small_tmp) and len(big_near_tmp) > len(small_near_tmp):
                ...
                if len(big_near_tmp) > 1 and big_near_gap_tmp[0] == big_near_gap_tmp[-1]:
                    for v in big_near_tmp:
                        arr.add(v)
            if len(small_near_tmp) >= len(big_tmp):
                if small_near_gap_tmp[-1] > -0.0007:
                    arr.add(small_near_tmp[-1])
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    pos = nearGapStat[-0.01] - 1
                    gapAndConsecutive(arr, small_near_tmp, small_near_gap_tmp, pos, left=True)
                    if noConsecutiveGap(small_near_gap_tmp):
                        if small_near_gap_tmp[-1] > -0.006:
                            arr.add(small_near_tmp[-1])
                if nearGapStat[-0.01] == 0:
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=True)
            if len(small_tmp) >= len(big_tmp):
            ...
            if len(big_tmp) - len(small_tmp) >= -1 and len(big_tmp) - len(small_near_tmp) >= -1:
                if len(big_tmp) > 1 and big_gap_tmp[0] == big_gap_tmp[-1]:
                    for v in big_tmp:
                        arr.add(v)
                if len(big_tmp) == 2:
                    if gapStat[0.02] == 1:
            ...
            if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) >= len(small_near_tmp):
                if nearGapStat[0.01] == 0:
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.03, reverse=True, consecutive_at_left=True, choose_single=True)
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                    findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.03, reverse=True, consecutive_at_left=False, choose_single=False)
                if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                    if big_near_gap_tmp[0] < 0.0002:
                        arr.add(big_near_tmp[1])
                    pos = len(big_near_tmp) - nearGapStat[0.01]
                ...
                if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                    ...
                    if noConsecutiveGap(small_near_gap_tmp):
                        if len(small_near_tmp) > 2:
                            if small_near_gap_tmp[-1] > -0.002 and small_near_gap_tmp[-3] < -0.006:
                                arr.add(small_near_tmp[-2])
                            if small_near_gap_tmp[-1] > -0.0028 and small_near_gap_tmp[-3] < -0.0052:
                                arr.add(small_near_tmp[-1])
                if nearGapStat[-0.01] == 0:
                    if len(small_near_tmp) == 3:
                        if small_near_gap_tmp[0] < -0.008 and small_near_gap_tmp[2] > -0.003:
                            arr.add(small_near_tmp[1])
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                    findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=True)
            if len(small_tmp) >= len(big_tmp):
        ...
        if len(small_tmp) == gapStat[-0.02]:
            ...
            if midJudge(small_tmp, gapStat, -0.03, False):
                findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=True)
                findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=True)
        ...
        if len(small_near_tmp) >= len(big_tmp) and len(small_near_tmp) >= len(big_near_tmp):
            ...
            if midJudge(small_near_tmp, nearGapStat, -0.01, False):
                ...
                if small_near_gap_tmp[pos] < -0.009:
                    arr.add(small_near_tmp[pos + 1])
                    if small_near_gap_tmp[pos + 1] - small_near_gap_tmp[pos] < 0.0001:
                        arr.add(small_near_tmp[pos])
                else:
                    gapAndConsecutive(arr, small_near_tmp, small_near_gap_tmp, pos, left=False)
                gapAndConsecutive(arr, small_near_tmp, small_near_gap_tmp, pos - 1, left=True)
                findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
            if small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                for i in range(len(small_near_tmp)):
                    arr.add(small_near_tmp[i])
            if nearGapStat[-0.01] == 0:
                ...
                findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=True)
            if len(small_near_tmp) == nearGapStat[-0.01]:
                findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                findTurn(arr, small_near_tmp, small_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
            if len(small_near_tmp) == 2:
                if nearGapStat[-0.01] == 1:
        ...
        if len(big_tmp) > len(small_tmp) and len(big_tmp) > len(small_near_tmp):
            ...
            if midJudge(big_tmp, gapStat, 0.03, True):
                ...
                gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos, left=True)
                gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos + 1, left=False)
                if big_gap_tmp[-1] > 0.0349 and big_gap_tmp[-1] < 0.0355:
                    arr.add(big_tmp[-1])
            if midJudge(big_tmp, gapStat, 0.03, False):
                pos = len(big_tmp) - gapStat[0.03]
                gapAndConsecutive(arr, big_tmp, big_gap_tmp, pos - 1, left=True)
                ...
                for i in range(pos + 1, len(big_tmp)):
                    if big_gap_tmp[i] - big_gap_tmp[pos] < 0.0001:
                        arr.add(big_tmp[i])
                if big_gap_tmp[-1] > 0.0355 and big_gap_tmp[-2] < 0.0338:
                    arr.add(big_tmp[-1])
                if big_gap_tmp[pos - 1] > 0.0288:
                    if pos - 2 > -1:
                        arr.add(big_tmp[pos - 2])
            if midJudge(big_tmp, gapStat, 0.04, False):
                pos = len(big_tmp) - gapStat[0.04]
            ...
            if len(big_tmp) == gapStat[0.02]:
                ...
                if gapStat[0.03] == 0:
                    findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=True)
                if big_gap_tmp[0] < 0.022 and big_gap_tmp[2] > 0.027:
                    arr.add(big_tmp[1])
            if gapStat[0.02] == 0:
                findTurn(arr, big_tmp, big_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=False, choose_single=False)
        if len(big_near_tmp) >= len(small_tmp) and len(big_near_tmp) >= len(small_near_tmp):
            ...
            if midJudge(big_near_tmp, nearGapStat, 0.01, False):
                ...
                if big_near_gap_tmp[pos] < 0.012:
                    if pos + 1 < len(big_near_tmp):
                        arr.add(big_near_tmp[pos + 1])
                if noConsecutiveGap(big_near_gap_tmp):
                    if big_near_gap_tmp[0] < 0.007 and big_near_gap_tmp[1] >= 0.0082:
                        arr.add(big_near_tmp[0])
                    if big_near_gap_tmp[0] < 0.0055 and big_near_gap_tmp[1] >= 0.0062:
                        arr.add(big_near_tmp[0])
                if big_near_gap_tmp[0] < 0.0035 and big_near_gap_tmp[1] > 0.005:
                    arr.add(big_near_tmp[0])
                gapAndConsecutive(arr, big_near_tmp, big_near_gap_tmp, pos - 1, left=True)
                if big_near_gap_tmp[0] < 0.004 and big_near_gap_tmp[2] > 0.007:
                    arr.add(big_near_tmp[1])
                findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=True, choose_single=False)
                findTurn(arr, big_near_tmp, big_near_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=False, consecutive_at_left=False, choose_single=True)
            if nearGapStat[0.01] == 0:
        ...
        if len(small_near_tmp) >= len(big_near_tmp):
            if len(small_near_tmp) == 2:
                ...
                if small_near_gap_tmp[1] > -0.002:
                    arr.add(small_near_tmp[0])
                if nearGapStat[-0.01] == 0:
                    if small_near_gap_tmp[1] > -0.0003:
                        arr.add(small_near_tmp[1])
            if len(small_near_tmp) > 1 and small_near_gap_tmp[0] == small_near_gap_tmp[-1]:
                for v in small_near_tmp:
                    arr.add(v)
    ...
    #大:接近:小很均衡
        ...
        if midJudge(small_tmp, gapStat, -0.03, False):
            findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=False)
        if midJudge(small_tmp, gapStat, -0.03, True):
            findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=True)
            findTurn(arr, small_tmp, small_gap_tmp, approach_threshold=0.0001, gap_threshold=0.01, reverse=True, consecutive_at_left=True, choose_single=False)
            pos = gapStat[-0.03] - 1
